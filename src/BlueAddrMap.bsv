...
interface AddrMapDecoder_ifc#(numeric type aw);
    method AddrMapHit_t#(aw) lookup(Bit#(aw) address);
    method AddrMapHit_t#(aw) lookup_span(Bit#(aw) address, Bit#(aw) bytes);
endinterface
...
module [Module] create_addr_map#(AddrMapCtx_t#(aw, endpoint, ifc) ctx)(AddrMap_ifc#(aw, ifc));
    ...
    function AddrMapHit_t#(aw) decode_span(Bit#(aw) address, Bit#(aw) bytes);
        ...
        for (Integer i = 0; i < List::length(targets); i = i + 1) begin
            ...
            Bit#(TAdd#(aw, 1)) target_end = fromInteger(target.base + addr_map_target_length(target));
    ...
    interface AddrMapDecoder_ifc decoder;
        method AddrMapHit_t#(aw) lookup(Bit#(aw) address) = decode_span(address, 1);
        method AddrMapHit_t#(aw) lookup_span(Bit#(aw) address, Bit#(aw) bytes) = decode_span(address, bytes);
    endinterface
